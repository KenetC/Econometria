...
coeficientes <- do.call(rbind, lapply(resultados_summary, function(summ) {
  coef_table <- coef(summ)[c(1,2,3),c(1,2)]  # Tabla de coeficientes
  cbind(coef_table[, 1][[1]],  # Coeficientes
        coef_table[, 1][[1]] - 1.96 * coef_table[, 2],  # Límite inferior (95% CI)
        coef_table[, 1][[1]] + 1.96 * coef_table[, 2])  # Límite superior (95% CI)
}))

# Convertir a data frame para facilitar el trabajo
coef_df <- as.data.frame(coeficientes)
colnames(coef_df) <- c("Coef", "Lower", "Upper")
coef_df$tau <- taus  # Añadir los cuantiles (tau) TENER CUIDADO

ggplot(coef_df %>% filter(variable == "anios_ed"), aes(x = tau, y = Coef)) +
  geom_line() +  # Línea de los coeficientes
  geom_ribbon(aes(ymin = Lower, ymax = Upper), alpha = 0.2) +  # Intervalos de confianza
  labs(x = "Cuantil (tau)", y = "Coeficiente de anios_ed", 
       title = "Coeficientes de la Regresión Cuantil y sus Intervalos de Confianza") +
  theme_minimal()
